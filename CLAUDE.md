...
**Issue the first analyst invite to the closed beta.**

The platform is deployed and the owner can self-test end-to-end. The auth-hardening tiers gated on this milestone are all shipped (Tiers 1, 2, 4-lite, 5 — see [`CHANGELOG.md`](CHANGELOG.md) v0.0.2 + v0.0.6), the S3 bucket-level hardening (versioning + Object Lock default rule GOVERNANCE/365d + CORS `GET`/`HEAD` for `https://vidit.app`) is in place, the weekly `pg_dump` → S3 cron is running with a verified restore drill on the books, and `v0.1.0` is tagged + deployed. What's left before the invite is owner-side: an end-to-end pass on `vidit.app` and a one-page analyst brief.
...
When in doubt about whether a task is in scope, check `next.md` first — its four milestone tables (M1 visibility → M2 open source → M3 open beta → M4 public v1) are the ordered plan.
...
- **Security hardening is a hard gate before any second human gets access.** Shipped: front door (rate-limits, env-driven CORS, JWT-secret hard-fail), cookie session + double-submit CSRF (Bearer path removed in v0.0.9), HSTS + `auth_events` audit log, server-side Tiptap sanitisation, EXIF strip + sha256 content hashes on every upload. Deferred to public launch: 15-min access tokens + refresh tokens (Tier 3) and CSP `script-src 'self'` + `report-uri` (full Tier 4). See [`docs/next.md`](docs/next.md) → *M3 — Open beta*.
