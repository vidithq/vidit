...
silently rewrite the record. ``events.revision_no`` says which version the live
row is (every existing row backfills to 1, the version it was published as), and
``event_revisions`` holds the versions it superseded: one append-only row per
edit, carrying the editable fields as they stood plus who edited and when.

``revision_no`` is unique per event, so the append-only writer cannot file two
...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.UniqueConstraint("event_id", "revision_no", name="uq_event_revisions_event_no"),
    )


def downgrade() -> None:
    op.drop_table("event_revisions")
    op.drop_column("events", "revision_no")
