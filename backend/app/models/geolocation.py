...
GeolocationStatus = Literal["requested", "detected", "geolocated", "closed"]
STATUS_REQUESTED: GeolocationStatus = "requested"
STATUS_DETECTED: GeolocationStatus = "detected"
STATUS_GEOLOCATED: GeolocationStatus = "geolocated"
STATUS_CLOSED: GeolocationStatus = "closed"
...
class GeolocationClaim(Base):
    ...
    gates the event's lifecycle, and the ``geolocation_id`` cascade drops claims
    on hard-delete.
    """

    __tablename__ = "geolocation_claims"

    geolocation_id: Mapped[uuid.UUID] = mapped_column(
        ForeignKey("geolocations.id", ondelete="CASCADE"), primary_key=True
    )
    ...
    geolocation = relationship("Geolocation", back_populates="claims")
    ...
    __table_args__ = (
        ...
        Index("ix_geolocation_claims_user_id", "user_id"),
    )
...
class Geolocation(Base):
    ...
    ``status`` (see ``GeolocationStatus``) is the lifecycle. ``location`` is an
    ...
    ``ck_geolocations_location_status``. Fulfilling a request is a single
    ...
    # ``status`` by ``ck_geolocations_location_status``.
    ...
    claims = relationship(
        ...
        order_by="GeolocationClaim.created_at.desc()",
    )
