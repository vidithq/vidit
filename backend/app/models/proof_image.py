...
class ProofImage(Base):
    ...
    Inserted when the editor uploads an image; `event_id` is set later,
    when the form is submitted and the URL survives sanitization. Rows with
    `event_id IS NULL` past a grace period are orphans (abandoned form),
    ...
    __table_args__ = (
        ...
        Index("ix_proof_images_event_id", "event_id"),
        # Reaper scans orphans (event_id IS NULL) by created_at; the
        # partial index keeps it cheap as linked rows accumulate.
        Index(
            "ix_proof_images_orphans_created_at",
            "created_at",
            postgresql_where="event_id IS NULL",
        ),
