...
`routers/geolocations/*` parse the multipart forms into clean Python types and
...
`routers/geolocations/_common.py` (`_GEOLOCATION_ERROR_STATUS`) — keep in sync
when adding a code.
"""
...
async def submit(
    ...
    * ``requested`` — an open call anyone may answer: ``author_id`` (the
    ...
    Raises :class:`GeolocationStateError` (409) off ``requested`` / ``detected``,
    ...
    if geo.status not in (STATUS_REQUESTED, STATUS_DETECTED):
        raise GeolocationStateError("Only requested or detected events can be geolocated")
