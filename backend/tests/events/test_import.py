...
def _detections(db, owner) -> list[Event]:
    db.expire_all()
...
def test_your_own_post_creates_a_detection(db, linked_author):
    ...
    [detection] = _detections(db, linked_author)
    assert str(detection.id) == body["created"][0]
    assert detection.status == STATUS_DETECTED
    assert detection.detected_from_url == _url(COORD_ID)
    assert detection.title == "Strike on the depot"


def test_several_coordinates_land_several_detections_and_a_warning(db, linked_author):
    ...
    assert len(_detections(db, linked_author)) == 2
...
def test_a_post_with_no_coordinate_creates_nothing_and_names_why(db, linked_author):
    ...
    assert _detections(db, linked_author) == []


def test_pasting_the_same_post_twice_never_duplicates_the_detection(db, linked_author):
...
def test_pasting_an_edited_post_overwrites_the_open_detection(db, linked_author, monkeypatch):
    first = _post(linked_author, COORD_ID)
    assert first.status_code == 200, first.text
    ...
    [detection] = _detections(db, linked_author)
    assert detection.title == "Strike on the fuel depot"
