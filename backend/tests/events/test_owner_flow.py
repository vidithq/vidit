...
def test_submit_requires_authentication(db, author):
    ...
    response = client.post(f"/api/v1/events/{geo.id}/submit", data=_submit_form())
    assert response.status_code == 401


def test_submit_returns_404_for_unknown_id(author):
    response = client.post(
        f"/api/v1/events/{uuid.uuid4()}/submit",
        data=_submit_form(),
...
def test_reject_requires_authentication(db, author):
    geo = _detected(db, author)
    response = client.post(f"/api/v1/events/{geo.id}/reject")
    assert response.status_code == 401


def test_reject_returns_404_for_soft_deleted(db, author):
    geo = _detected(db, author, deleted=True)
    response = client.post(f"/api/v1/events/{geo.id}/reject", headers=login_as(client, author))
    assert response.status_code == 404


def test_reject_returns_403_when_not_author(db, author, second_user):
    geo = _detected(db, author)
    response = client.post(f"/api/v1/events/{geo.id}/reject", headers=login_as(client, second_user))
    assert response.status_code == 403
...
def test_reject_soft_deletes_detected(db, author):
    ...
    geo = _detected(db, author)
    geo_id = geo.id
    response = client.post(f"/api/v1/events/{geo_id}/reject", headers=login_as(client, author))
    assert response.status_code == 204
    ...
    assert client.get(f"/api/v1/events/{geo_id}").status_code == 404
...
def test_reject_invalidates_points_cache(db, author):
    ...
    assert client.get("/api/v1/events/points").headers.get("x-cache") == "MISS"
    assert client.get("/api/v1/events/points").headers.get("x-cache") == "HIT"
    client.post(f"/api/v1/events/{geo.id}/reject", headers=login_as(client, author))
    after = client.get("/api/v1/events/points")
