"""The owner review queue — ``GET /geolocations/review-queue``.
...
evidence and computes validation-readiness client-side. Scoping is to
``current_user`` — the endpoint ignores any URL username. Shared fixtures live
...
_URL = "/api/v1/geolocations/review-queue"
...
def test_review_queue_requires_authentication(db, author):
    _detected(db, author)
    response = client.get(_URL)
    assert response.status_code == 401


def test_review_queue_empty_for_user_without_detections(author):
    response = client.get(_URL, headers=login_as(client, author))
    assert response.status_code == 200
    assert response.json() == {"items": [], "total": 0, "page": 1, "per_page": 20}


def test_review_queue_returns_only_callers_live_detected(db, author, second_user):
    """Only the caller's live ``detected`` rows: not a validated row, not a
...
def test_review_queue_includes_media_and_tags(db, author, conflict_tag, capture_source_tag):
...
def test_review_queue_ordered_newest_first(db, author):
...
def test_review_queue_paginates(db, author):
    for _ in range(3):
        _detected(db, author)
...
def test_review_queue_caps_per_page(author):
...
def test_review_queue_clamps_out_of_range_paging(author):
