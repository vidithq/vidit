...
Queue Wayback archival for every live event link that has no [`source_archives`](data-model.md#source_archives) row: the catalog backfill behind create-time archival. Walks live events oldest first, capped per call. Enqueue only, so the response returns immediately whatever the catalog size; the worker paces the captures (see [`ingestion.md`](ingestion.md#source-archival)). Idempotent per link, so a repeat call only picks up what has been written since. Audited as `maintenance_enqueue_source_archival`.
