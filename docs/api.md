...
`bbox` is **required**: the response covers one viewport, so payload size tracks
the area asked for, not the size of the catalog, and no single call returns
every point on the platform. Unlike on `GET /events`, an empty `?bbox=` is a
...
| Param | Type | Description |
|-------|------|-------------|
| `bbox` | string, **required** | `south,west,north,east` (four comma-separated floats), same shape and validation as on `GET /events`: latitudes in [-90, 90], longitudes in [-180, 180], south ≤ north, west ≤ east. Missing, empty, or malformed → 422. Boxes crossing the antimeridian are not modelled; split them into two calls. |
...
| 422 | `bbox` missing or malformed, or a `media` / `author` value outside its domain |
...
`GET /events` and `GET /events/points` are intentionally **unpaginated**. `/events` caps its result set with `?limit=` (200 max); `/events/points` bounds its own with the required `bbox`, so the viewport decides the payload.
