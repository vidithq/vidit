...
**The description is a document, and its projection is a column.** The pair is written together in `services/collections`, on the create and on every edit, so nothing can store a document whose stored projection describes different words. See [`api.md`](api.md#collections) for the node and mark allowlist, and for what a 422 means.
