...
- **Controls.** [`<Switch>`](../frontend/src/components/ui/Switch.tsx) is the one boolean toggle (`md` settings rows, `sm` map filter rows; `as="span"` when a whole-row parent owns the click). [`<SegmentedControl>`](../frontend/src/components/ui/SegmentedControl.tsx) is the exclusive-choice bar (submit type, admin delete mode, the detections queue filter; `tone="danger"` for a destructive active option, `title` on an option for hover text on that option's own button, which is what a one-word label leans on). `<Input icon>` overlays a leading icon (the search box). [`<Select>`](../frontend/src/components/ui/Input.tsx) is the pick-one-from-a-short-list field, running `<Input>`'s own shape recipe under a custom caret, so a select and a text field on one row can't drift. [`<LinkListInput>`](../frontend/src/components/ui/LinkListInput.tsx) is the ordered list of URL fields (the submit and edit forms' Secondary sources): one `<Input>` per row with a per-row remove, an add button that disables at its `max`, and a `locked` read-only mode.
...
- **Source links.** [`<SourceLabel>`](../frontend/src/components/ui/SourceLabel.tsx) renders a stored source URL as its host, with italic fallbacks for a demo sentinel, a null URL, and a value the parser gives no host for. [`<ArchivedCopies>`](../frontend/src/components/ui/ArchivedCopies.tsx) is the icon pair beside it on the event detail surfaces (the full page and the map side panel alike), one affordance for the primary source, the provenance link and every secondary mirror. One icon per archiving service, accent and clickable where that service holds a copy, greyed and inert where it does not, so a missing copy is shown rather than hidden. The glyphs are lucide marks (a clock-with-arrow for the Wayback Machine's replay, a box for the archive.today snapshot), never the services' own logos, and the name carries the identity. It renders nothing when the link has no archival row at all, except under `pendingPublication`, the draft variant a caller sets from the event's status: a `detected` draft's links are queued only at publication, so the pair renders greyed and says so instead of leaving the row bare. Every glyph on the page looks alike, so a hover title carries the state and the accessible name carries the target as well: `PRIMARY_SOURCE_DESCRIPTION` for the source, `DETECTED_FROM_DESCRIPTION` for the provenance link, `mirrorDescription(host, index, total)` for a mirror, which leads with the position whenever the list holds more than one and falls back to a literal for a hostless URL, plus "archiving in progress" while a copy may still land, "archiving failed" once the payload flags the link unavailable, and "archived when published" on a draft.
- **Small assemblies.** [`<AuthorByline>`](../frontend/src/components/ui/AuthorByline.tsx) is "by @user". `avatar` leads with the author's picture instead of the "by " prefix (picture plus handle already reads as a signature), on the detail pages and the map side-panel header. [`<Dot>`](../frontend/src/components/ui/Dot.tsx) is the accent notification dot. [`<EmptyState>`](../frontend/src/components/ui/EmptyState.tsx) owns the empty-state grammar (`boxed` / `plain` / `invite`). The anchored-popover machinery (pin, hover, outside-click / Escape dismiss, portal plus viewport clamp) is [`usePinnedPopover`](../frontend/src/hooks/usePinnedPopover.ts), used by `FieldHelp`.
...
7. **Coverage**: the analyst's geolocations on a map (`ProfileMap`), the shared `<Map>` fed by `/events/points?author=…` with an explicit world `bbox` (`WORLD_BOUNDS` serialised through `toBboxParam`; the endpoint requires one), camera fitted to the returned points. It maps both live statuses, `geolocated` and `detected`, which the shared map already paints in its two point shades, so the drafts behind the work show as drafts. `hide_demo` drops the demo pool. The count beside the heading splits the same way ("N geolocated, N detected on the map"), under the status names the Insights card above uses, so no two numbers on the page count the same thing differently. It is not the `Submitted` tile's figure: that tile counts the whole body of work, drafts included, and this one counts located rows. Hidden when the analyst has no located events.
...
**The detections queue is a list, and the work happens in review mode.** `/profile/{username}/detections` is a dense list, one [`DetectionQueueRow`](../frontend/src/components/detections/DetectionQueueRow.tsx) per draft: a thumbnail, the title, the event date, the source host, and one state badge. The badge describes the evidence, not completeness (`batchCompletionBlockers` in [`lib/events.ts`](../frontend/src/lib/events.ts), which mirrors the server floor). It reads **Ready to review** when the machine found everything it could and the draft is waiting on the judgment a review supplies, the conflict and the capture source, so it takes `<Pill>`'s outline `secondary` tone rather than the filled accent: a draft that still needs two picks must not read as a finished one. Otherwise it names the missing piece, or counts them (*Missing: 2 pieces*) once several names would outgrow a dense row. Both states carry hover text on the badge itself: what the state means, the missing pieces in full whether or not the badge collapsed them, and what to do next. A [`<SegmentedControl>`](../frontend/src/components/ui/SegmentedControl.tsx) narrows the page to ready or incomplete rows, client-side, since readiness is computed from the payload rather than asked of the server. Each of its three options carries hover text saying what it selects, in the badges' own words, because the labels are one word each. There are no inline controls: the whole row is one click, to the full edit form, which is where a draft missing evidence gets its manual pass. Pagination sits at the foot, as on every list.

**Review mode is one draft at a time.** *Start reviewing* opens `/profile/{username}/detections/review`, a route of its own so the flow is a link an analyst can keep and the browser's Back leaves it for the queue. The session loads one batch of the queue and steps through it locally, so a published row leaving the server-side queue cannot shift the position under the analyst. Each draft renders as its evidence on one side ([`<MediaGallery>`](../frontend/src/components/ui/MediaGallery.tsx) plus the proof body in [`ProofSection`](../frontend/src/components/ui/ProofSection.tsx)) and the point on the other (the shared [`<Map>`](../frontend/src/components/map/Map.tsx) carrying the draft's pin, over the same [`CoordinateInputs`](../frontend/src/components/geolocations/CoordinateInputs.tsx) the forms use, so the pin is adjustable and saves with the publish). Under them sit the four fields a publish still needs: the shared [`TitleField`](../frontend/src/components/geolocations/TitleField.tsx), the event date, the capture source as a [`<Select>`](../frontend/src/components/ui/Input.tsx), and the conflict through the shared [`ConflictTypeahead`](../frontend/src/components/ui/TagPicker.tsx). The two picks are **sticky**: the value chosen on one draft is what the next one opens with, because an import usually sits in one conflict, so the common pass is Publish, Publish, Publish. Chips stay the choice where the options are a taxonomy to browse ([`<TagPicker>`](../frontend/src/components/ui/TagPicker.tsx)); `<Select>` is for a dense row where one field is the choice.

Publishing runs the single-row [`POST /events/{id}/geolocate`](api.md#post-eventsidgeolocate) transition, so the server floor, the geolocator credit and the archival enqueue are the ones a submit from the full form gets, and it advances to the next draft. **Skip** moves on without writing; **Reject detection** is the same inline [`CloseEventForm`](../frontend/src/components/event/CloseEventForm.tsx) the edit form carries. All three are on the keyboard (Enter, S, X), standing down while a field holds focus, since the shortcuts are single letters. A draft missing evidence the review cannot supply (its source, its post time, its footage, its annotated proof) says which piece, disables Publish, and offers the full form by name; Skip moves past it. The session's header carries the position (*Draft n of m*, with the queue total beside it when the queue runs past one batch), the key legend, and the way back to the queue. The legend sits there rather than beside the actions it drives, so it is on screen from the first draft without a scroll.
...
[`FieldHelp`](../frontend/src/components/ui/FieldHelp.tsx) puts a `?` next to a field label, section heading, or detail row: a one-line explanation on hover or focus, pinned on click (touch can't hover), dismissed by outside-click or Escape (a real `role="tooltip"`, not a CSS `title`). It is **neutral grey, not accent** (meta help), the one sanctioned exception to the rule that clickable implies accent. Each `?` is `<FieldHelp concept="…" />`. The concept registry [`lib/fieldHelp.ts`](../frontend/src/lib/fieldHelp.ts) pairs each concept's `text` and `label` once (wording mirrors [`data-model.md`](data-model.md)), so the same concept explains a field on the submit form, the detail page or map panel, and the filter panel without drift. Sections carry no always-on subtitle, so the `?` is the single source of section help. Readers can hide every `?` through Settings → Display (per-browser), leaving only labels and fields.
