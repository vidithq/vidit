...
**One submit page, two types.** Geolocation and request creation share ~80% of their fields, so [`/submit`](../frontend/src/app/submit/page.tsx) carries both behind a **Geolocation | Request** toggle; the header stays uniform, the toggle owns the framing. `/requests/new` and the old `/geolocations/new` redirect here (query-preserving). Fulfilling a request (`?request_id=`) is always a geolocation, so the toggle hides.
