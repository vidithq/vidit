...
Submit forms mirror their detail page's reading order — **title → media → location → details → proof** — so an analyst maps "what I fill in" to "what readers see". **Title leads** (it's the detail page's heading). The geolocation form ([`geolocations/new`](../frontend/src/app/geolocations/new/page.tsx)) runs Title → Media ([`EvidenceUploader`](../frontend/src/components/geolocations/new/EvidenceUploader.tsx)) → Location ([`LocationPicker`](../frontend/src/components/geolocations/new/LocationPicker.tsx), coordinates) → Details ([`DetailsFields`](../frontend/src/components/geolocations/new/DetailsFields.tsx): event date, source date, source URL) → Tags ([`TagPicker`](../frontend/src/components/ui/TagPicker.tsx)) → Proof ([`ProofEditorPanel`](../frontend/src/components/geolocations/new/ProofEditorPanel.tsx)); the bounty form ([`bounties/new`](../frontend/src/app/bounties/new/page.tsx)) runs Title → Media → Details → Tags. The tweet-import shortcut ([`TweetImportBanner`](../frontend/src/components/geolocation/TweetImportBanner.tsx)) is a **collapsed secondary action** above the fields, not a field — so it reads as an action and its no-X-link nudge stays hidden until opened. To keep the mirror tight, the geolocation detail page shows the coordinates directly under the Location map (not as a Details row), matching the form's Location section; the map side-panel, which renders no Location map, keeps the coordinate row.
