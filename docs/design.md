...
- **Live progress.** [`<ProgressSteps>`](../frontend/src/components/ui/ProgressSteps.tsx) is the vertical stepper for a live multi-step operation (the archive import): check for done, highlighted disc + progress bar for the active step (determinate when a 0..1 `progress` exists, an indeterminate pulse otherwise), muted for pending. `failed` turns the active step into the red failure marker; the message itself stays in the form's `FORM_ERROR_BANNER`.
