...
**Media.** [`<MediaGallery>`](../frontend/src/components/ui/MediaGallery.tsx) is the detail-surface block (geolocation detail, map panel, bounty detail): 2-up `hero` grid on the page, stacked `thumbnail` tiles in the panel, videos postered via `#t=0.1` + `preload="metadata"`. The card-sized media slot (real media or a marked no-media box; no generated stand-ins) is private to `EntityCard`, its only consumer.
