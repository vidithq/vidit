...
**Coordinates travel as a pair.** Both pairs (the subject point and the camera position) are the shared [`CoordinateInputs`](../frontend/src/components/geolocations/CoordinateInputs.tsx). A paste that reads as a pair fills both fields whichever one received it: a decimal `lat, lng` (comma or space separated, degree signs tolerated) or a Google Maps URL (`@lat,lng`, `q=`, `query=`). Anything else pastes as ordinary text. Once the pair parses within bounds, a **View on Maps** link (new tab) and a `<CopyButton>` appear under the fields, so a coordinate can be checked against satellite imagery and carried away while it is being typed; the event detail page's coordinates row carries the same two affordances. The parser, the bounds, the 6-decimal rendering and the map-link builder have one home, [`lib/coordinates.ts`](../frontend/src/lib/coordinates.ts).

**Media can come from the source post.** When a detection's source URL is an X post and its media slot is empty, the Source media section offers **Download media from source**: it reads the post through the tweet-import endpoints and stages the first media the CDN proxy serves, exactly as a hand-picked file is staged, uploading with the rest of the form at submit. The button is absent once a media is kept or staged, one source media being all an event holds.
