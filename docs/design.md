...
- **Source links.** [`<SourceLabel>`](../frontend/src/components/ui/SourceLabel.tsx) renders a stored source URL as its host, with italic fallbacks for a demo sentinel, a null URL, and a value the parser gives no host for. [`<ArchivedCopies>`](../frontend/src/components/ui/ArchivedCopies.tsx) is the icon pair beside it on the event detail surfaces, one affordance for the primary source and every secondary mirror. One icon per archiving service, accent and clickable where that service holds a copy, greyed and inert where it does not, so a missing copy is shown rather than hidden. The glyphs are lucide marks (a clock-with-arrow for the Wayback Machine's replay, a box for the archive.today snapshot), never the services' own logos, and the name carries the identity. It renders nothing only when the link has no archival row at all. Every glyph on the page looks alike, so the accessible name carries both the target and the state: `PRIMARY_SOURCE_DESCRIPTION` for the source, `mirrorDescription(host, index, total)` for a mirror, which leads with the position whenever the list holds more than one and falls back to a literal for a hostless URL, plus "archiving in progress" while a copy may still land and "archiving failed" once the payload flags the link unavailable.
...
4. **Insights** (`ProfileInsights`): the shape-of-work card from `GET /users/{username}/stats`. It has its own second `<StatGrid>` (status split, media) plus top conflicts, capture sources, and the 12-month activity bars. It's hidden entirely for a profile with no events, so the strip above is what a new account shows.
5. **Coverage**: the analyst's geolocations on a map (`ProfileMap`), the shared `<Map>` fed by `/events/points?author=…` with an explicit world `bbox` (`WORLD_BOUNDS` serialised through `toBboxParam`; the endpoint requires one), camera fitted to the returned points. It shows published work only: `hide_demo` drops the demo pool, and the payload is narrowed to `geolocated`, so machine drafts neither move the camera nor enter the count beside the heading. Hidden when the analyst has no located events.
6. **Recent submissions**, then **linked accounts** (shown to any visitor, whenever the profile carries links), then the owner-only blocks (the detections queue entry, sign out).

Editing collapses that order. While the owner is editing, the page renders only the header fields, the bio, and the linked-accounts inputs. The read-only sections drop out for the duration, so every field sits between the header and the Save button in `actions`.
