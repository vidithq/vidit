...
The suite lives in [`frontend/e2e/`](../frontend/e2e) and [`frontend/playwright.config.ts`](../frontend/playwright.config.ts) runs it as two projects, Chromium at 375x812 and at 320x568, so every spec runs at both widths. It covers the four golden paths: open a shared event link and read it, browse and filter the map, fill the submit page in its single-form and X-post-import modes, and sign in and read settings. It also covers the request board, which is not a golden path but holds the narrowest content column in the product, about 150px at 320px, where a tag that does not wrap inside its chip scrolls the page sideways.

Each page takes the same three assertions, held in one module (`e2e/support/narrowLayout.ts`) rather than repeated per spec:

- `document.documentElement.scrollWidth` equals `document.documentElement.clientWidth`, so the page does not scroll sideways. `clientWidth` is the viewport's content box: measured against `window.innerWidth` instead, a runner that paints a classic scrollbar reads every page as 15px narrower than the window and the check passes with a real 15px overflow in it.
- The page's primary control is visible, scrolled into view, and wholly inside the viewport on both axes (`toBeInViewport({ ratio: 1 })`).
- Every visible editable field computes a `font-size` of at least 16px. A field is an `<input>`, `<textarea>`, `<select>` or `contenteditable` element that renders typed text: the `<input>` types that do not (`hidden`, `checkbox`, `radio`, `file`, `range`, `color`, and the button spellings `submit`, `button`, `reset`, `image`) are excluded, since none of them triggers the zoom.

There is no screenshot comparison, which is what keeps the job stable across runners with different fonts, and no retries: every assertion is geometric against a page served from fixtures, so a failure is a layout regression rather than a flake. A failing run keeps its traces under `frontend/test-results/`, which the CI job uploads as an artifact.

The two projects are Desktop Chrome at a phone viewport, not a device profile: what they measure is the CSS the width selects, so there is no touch emulation, no mobile user agent and no device pixel ratio.
