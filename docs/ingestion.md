...
The bot reads its own mentions and accepts exactly one shape: a **single tweet** carrying the strict inline format. The tagged tweet must hold all of:

- `T:` a title (non-empty)
- `C:` one decimal coordinate pair (`lat, lng`, comma or space separated, nothing else on the line; bounds-checked by the same `validate_coordinates` every other path runs)
- `S:` a line holding a source link, which must resolve through [the shared source vocabulary](#the-contract): a quote, or a sole X-status / Telegram / YouTube footage link

Marker lines match at line starts, case-insensitively (`t:` works). Every other line becomes the event's proof text; the marker lines, the raw coordinate string, the `@viditbot` tag, and `t.co` shortlinks never reach the stored draft. Media attached to the tagged tweet lands as the analyst's annotation (`role=proof`; images, embedded in the proof document). A tweet missing any required part produces nothing (see the failure reply below): free-text coordinate detection is **not** a fallback on the bot path; that vocabulary stays the archive backfill's and the human paste's.

**What the `S:` source yields** (`detect_structured` in [`detect.py`](../backend/app/services/tweet_ingest/detect.py)): an X status resolves via syndication (at most one extra free fetch per mention) to its canonical URL, its post date as `source_posted_at`, and its media as `role=source`; a quote is the same signal without the extra fetch. A public `t.me` post resolves through the existing embed chase to its post date, plus the footage media when the embed serves it (a sensitive post serves date only, a valid outcome). A YouTube link is stored as `source_url` with no media fetch. A link outside the vocabulary (an article, a profile) resolves nothing, so the mention fails the format.

**Single tweet, no parent rollup.** The bot fetches only the tagged tweet; a coordinate or source living in an ancestor does not count. The archive backfill keeps its self-thread stitching unchanged (its threads are same-author by construction, see [Archive formats](#archive-formats)).
...
|---|---|---|
| Mention received | Like the tagged tweet | Author is a linked live account only; `no_account` stays fully silent |
| Import succeeded | In-thread reply: bare event ref + warnings | Always (budget permitting) |
| Import failed (format incomplete or invalid) | In-thread reply: why + the `T:` / `C:` / `S:` format | Author linked AND the tagged tweet is not itself a reply to the bot (the loop guard: a courtesy answer to the bot's own reply auto-mentions it and must not earn another reply, forever) |
| Any | Retweet | Never (deliberately out) |
