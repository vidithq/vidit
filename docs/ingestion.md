...
| `request_not_possible` | The caller asked for a request and the coordinate-less thread can never yield one: it declares a source the chase does not read, or it carries no footage. `coords_missing` travels underneath, so only that caller sees this. |
