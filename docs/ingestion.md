...
| Import failed (format incomplete or invalid) | In-thread reply mirroring the success shape (header, one ⚠ line per problem, footer): the ❌ header names where the bot read (the tagged post, or the parent for a relay), the ⚠ line carries the diagnosed reason + the one fix to make; no recited lesson (the full guide lives behind the bio link) | Author linked AND the tagged tweet is not itself a reply to the bot (the loop guard: a courtesy answer to the bot's own reply auto-mentions it and must not earn another reply, forever) |
| Anything else | Nothing | `no_account` and every unlinked author stay fully silent |

Reply text is **linkless, never a URL or auto-linkable domain**: X bills a link-carrying post ~13x a plain one, so the clickable link lives in the bot bio. Every reply is also **unique per mention** (the success ref, and a short mention tail on failures): X refuses a tweet identical to a recent one (403 duplicate content), which would otherwise eat a repeated diagnosis; that specific 403 is logged without paging. The success reply warns when the source's post date came back unknown (an off-vocabulary link or a failed chase: the provisional event date then anchors on nothing but the analyst's own post) and when the draft's media is already known on Vidit (exact `Media.sha256` equality; perceptual near-duplicate matching is a separate feature); it is single-draft by construction (one coordinate line per mention) and carries no missing-source warning (a sourceless draft cannot pass the format).
