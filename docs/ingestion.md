...
| Import failed (no coordinates detected) | In-thread reply: why + the expected format | Author linked AND the tagged tweet is not itself a reply to the bot (the loop guard: a courtesy answer to the bot's own reply auto-mentions it and must not earn another reply, forever) |
| Any | Retweet | Never (deliberately out) |

Reply text is **linkless, never a URL or auto-linkable domain**: X bills a link-carrying post ~13x a plain one, so the clickable link lives in the bot bio. Success warnings ride along: no source quote or footage link (blocks the `geolocate` promotion), and media already known on Vidit (exact `Media.sha256` equality; perceptual near-duplicate matching is a separate feature).
