...
**The chase.** The X and Telegram vocabulary decides what gets *fetched*, never what gets *stored*. A sole X status candidate resolves through syndication, which supplies the canonical URL, the post date and the media. A sole public `t.me` post resolves through its embed, which supplies the post date, plus the media when the embed serves it; a sensitive post serves neither. Every other candidate fills `source_url` link-only, with no date and no media. An ambiguous thread chases nothing.
...
- **Third-party quotes and status links**: resolved by chasing the referenced tweet ID through syndication, when chasing is enabled for the import. With the chase off (the pure-disk read), the link is still the source, stored link-only.
