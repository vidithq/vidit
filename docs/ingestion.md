...
- **The engine** is [`resolve.py`](../backend/app/services/tweet_ingest/resolve.py) (a thread resolves to a `ResolvedThread`), [`detect.py`](../backend/app/services/tweet_ingest/detect.py) (one `DetectedGeoloc` per coordinate, plus the warnings), and [`detection.import_threads`](../backend/app/services/detection.py) (the one write path from a thread to `detected` rows).
