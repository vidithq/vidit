...
A chase that came back empty still says why. Each chaser answers one of four outcomes (`chased`, `not_accessible`, `transient_failure`, `no_target`), and a failure is stamped on the record that declared the target, which is how the resolution, itself pure, tells a source with no footage from a source nothing could read. Only the transient one changes what the analyst is told: the draft carries `source_fetch_failed` instead of `source_footage_missing`.

**Retries.** Every outgoing fetch of the ingestion shares one schedule ([`retry.py`](../backend/app/services/tweet_ingest/retry.py)): three attempts, pausing 1 s then 3 s, never sleeping more than 6 s in total, honouring a longer `Retry-After` from the upstream within that budget. Only a throttled or unreachable upstream earns a retry; a post that is gone, a restricted one and a payload that will not parse come back on the first attempt, since no number of attempts changes any of them.

**Media format.** An imported photo is re-encoded to one format at ingest, whichever entry read the post: `records.PHOTO_CONTENT_TYPE`, the format the display derivatives (`_hero`, `_thumb`) already use. These are machine-fetched bytes rather than an analyst's upload, so nothing is lost by normalizing them, and no entry derives a photo's type from a payload field or a filename, which is what let one PNG land under two different types depending on the entry. Videos are stored as fetched, the mp4 variant every payload reader picks. The per-media size caps (`MAX_IMAGE_SIZE`, `MAX_VIDEO_SIZE`) apply to the fetched bytes, before the re-encode; over-cap media is skipped and the draft still lands.
...
The footage and date warnings are dropped on a draft that already carries `source_ambiguous` or `source_missing`, since an empty source slot already says why there is neither footage nor date.
...
**One sentence per code.** Every code has exactly one wording, in `resolve.WARNING_MESSAGES` and `REFUSAL_MESSAGES`, and every surface reads it: the bot's reply adds the ⚠ and the length discipline, the outcome email adds the count of drafts, and the paste returns the sentence beside the code so the import panel renders what it is handed. A code added to the vocabulary without a sentence fails `test_engine_copy`. Branch on the code, which is stable; the sentence is prose.
...
The success reply carries one line per [warning](#warnings) the pass raised, in one fixed order, and decides none of them itself: which warnings a draft carries is the engine's and the write path's answer. Re-tagging is the repair for none of them, since it lands on the existing idempotency key and deduplicates; review is. The failure reply names one of the three refusals the engine can tell apart, or, when the write path raised on every draft rather than the engine refusing anything, states that the case is unexpected and points at the maintainers.
...
- **Photos and videos**: video capture takes the highest-bitrate mp4 variant the export saved. Each media entry names the file the export wrote under `tweets_media/`; a photo's stored format is the constant every entry uses (see [Media format](#the-contract)).
...
**The owner gate, before anything is downloaded.** A job runs only for a live, active account that carries a linked `x_handle`, which is the same requirement the two live entries make of an import's owner (the bot reads `detection.linked_owner`, the paste goes through `get_current_user`). A job whose owner was soft-deleted or deactivated between the upload and the claim lands `failed`, so a suspended account accrues no drafts. So does a job whose owner carries no handle: the handle is what every provenance permalink is written from and what the [own-status exclusion](#the-contract) compares a link against, and there is no fallback onto the Vidit username, which would fabricate links to an account that may be someone else's. The terminal states are `done`, with assemble counts stamped, and `failed`, with a terse `error`. Both states delete the staged object, so no live object accumulates: the bucket's versioning keeps a noncurrent copy until the lifecycle rule expires it (see [`engineering.md`](engineering.md#deployment)). Zip-shape validation happens only here. A malformed upload lands `failed` and triggers the failure email; the browser strip catches the common shapes before upload.
