...
A request the bot opened carries two of these codes: `source_date_unknown`, when the chase served no date for the source it named, and `duplicate_media`, raised by the same `_rows_with_duplicate_media` comparison the detections run, over the footage the request stored. The remaining codes cannot apply, since a request is born with a source and the footage that fills its source slot. Each reads the same sentence from the same table, in the bot's ✅ reply.
...
| No coordinate, on a footage-carrying thread whose source is an X status or a Telegram post | The same ✅ reply, naming a request rather than a detection | Author linked, and the owner holds no row for that post or that source |
