...
- `S:` **exactly one URL token, designating the source.** Two or more URL tokens on the line is a format failure. The token binds to the tweet's link entity (X wraps every pasted link as `t.co`; the entity correlation resolves it) and that entity must be an X status, a Telegram post, or a YouTube link, never the author's own post. When X converts the pasted status URL into the quote card and strips the trailing `t.co` from the text, the inline quote is the designated source; an `S:` line with no URL token and no quote is a format failure.
