...
The bot reads its own mentions and accepts one strict format in two forms: **inline** (the tagged tweet itself carries the markers) and **relay** (the tag sits in the analyst's direct reply to their own marker tweet, the reply carrying the footage; see below). The marker tweet must hold all of:

- `T:` a title (non-empty)
- `C:` one decimal coordinate pair and nothing else on the line. Inside the pair the grammar is lenient: optional `+`/`-` sign and optional `°` on each number, comma or whitespace between them. Bounds-checked by the same `validate_coordinates` every other path runs.
- `S:` **exactly one URL token, designating the source.** Two or more URL tokens on the line is a format failure. The token binds to the tweet's link entity (X wraps every pasted link as `t.co`; the entity correlation resolves it). Any bound link is accepted and stored as the source, whatever its host, with one exception: a link back to the author's own post (a cross-reference, not footage). When X converts the pasted status URL into the quote card and strips the trailing `t.co` from the text, the inline quote is the designated source; an `S:` line with no URL token and no quote is a format failure.

Marker lines match at line starts, case-insensitively (`t:` works); a marker takes its first non-empty value, so an empty `T:` line never shadows a real one below it. Every other line becomes the event's proof text; the marker lines, the raw coordinate string, and the `@viditbot` tag (configurable, `X_BOT_HANDLE`) never reach the stored draft. Links on proof lines never influence source resolution and never fail the mention: their opaque `t.co` tokens are expanded back to the real URL in the stored proof (a reference link survives readable); leftover tokens with no entity (the wrapper X appends for attached media) are stripped. Media attached to the tagged tweet lands as the analyst's annotation (`role=proof`): images only, embedded in the proof document; an attached **video is dropped** (the proof document embeds no video). A tweet missing any required part produces nothing (see the failure reply below): free-text coordinate detection is **not** a fallback on the bot path; that vocabulary stays the archive backfill's and the human paste's, including the archive's chase of footage links without any `S:` marker.
...
**The relay form** (`detect_relay` in [`detect.py`](../backend/app/services/tweet_ingest/detect.py)): for an `S:` link the chase vocabulary cannot fetch (TikTok, Instagram, an article), the analyst posts the marker tweet, then tags the bot in a **direct reply to it, attaching the re-uploaded footage**. One extra syndication fetch resolves the parent, which runs the same strict mapper as an inline mention; the parent must be the tagged author's own post, checked on the fetched handle, so a tag under someone else's post relays nothing. On top of that resolution: the reply's attached media becomes the source media (`role=source`), outranking any chased media while a chased post date is kept; the reply's non-marker text joins the proof as a caption; a media-less reply-tag resolves the parent exactly as if it had been tagged inline. Provenance (`detected_from_url`) anchors on the **parent**, so tagging both tweets collapses onto one idempotency key. This is the one sanctioned promotion of an analyst's own attachment into the source slot: the two-tweet structure is the explicit signal the resolution contract requires.
