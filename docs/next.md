...
The path from closed beta to public v1 is three milestones. The throughline is **progressive openness**: open the source + widen the closed-beta analyst pool in a single coordinated launch (M1 — the landing already ships, so the GitHub flip + pinned X tweet + cold-reach DMs all fire in the same window), open the doors (M2), call it done (M3). Reach is the current bottleneck and the community is skeptical of closed / unknown tools — M1 is the load-bearing trust signal.

| Milestone | What it delivers | What it unlocks |
|---|---|---|
| **M1 — Open source launch** *(now)* | Public vitrine at `vidit.app` already ships (landing + demo video); the open-source flip under AGPL-3.0, the pinned X tweet on `@vidithq`, and cold-reach DMs widening the closed-beta analyst pool fire as one coordinated launch | A skeptic clicking the tweet sees a working product and the code in one go; the "closed-source / vibe-coded" objection retires; the closed beta widens beyond the early testers in the same window |
| **M2 — Open beta** | Anonymous read + open self-registration, behind the safety + legal stack | Anyone can read and register; the trust filter becomes the curated layer on top |
| **M3 — Public v1** | Catalog density, search / social polish, cost tuning, beta framing removed | Full release |
...
The single coordinated launch that retires the "closed-source / unknown / vibe-coded" objection and widens the closed-beta analyst pool beyond the early testers already onboarded. The landing page already ships (the public vitrine + the demo video — see [CHANGELOG](../CHANGELOG.md) under *Unreleased* and *v0.2.0*); what's left is the open-source flip + the pinned X tweet on [`@vidithq`](https://x.com/vidithq) + cold-reach DMs to new analysts, all firing in the same window because the trust signal compounds when substance + code + presence arrive together. **100% open source under [AGPL-3.0](../LICENSE)** — no proprietary tier; the monetization layer on `vidit.app` is API rate limits + a small set of paid-only endpoints (see [`roadmap.md`](roadmap.md) → *Openness & transparency*). Orthogonal to the M2 (open beta) safety stack — this can land before that work begins.

| Pri | Area | Item | Why / how |
|---|---|---|---|
| P0 | Security | Session-lifecycle invalidation — `users.token_version` column + `get_current_user` claim check + bumps on logout / password change / password reset / soft-delete | Deferred from PR #100 (`/code-review ultra` follow-up). Clearing the session cookie on logout / password change / password reset / soft-delete doesn't invalidate the underlying token, so session revocation isn't immediate. **Implementation:** Alembic migration adds `users.token_version INTEGER NOT NULL DEFAULT 0`; [`services/auth.create_access_token`](../backend/app/services/auth.py) embeds it as a `tv` claim alongside `sub`; [`dependencies.get_current_user`](../backend/app/dependencies.py) compares the claim to the user row and 401s on mismatch; a `bump_token_version(user)` helper is called from the four mutation points above. The full Tier-3 refresh-token system in M2 supersedes this — ship the interim fix before the OSS-launch wave of cold-reach invites. Pair in the same PR: tighten the account-state guards on `/auth/reset-password` to match the mint side. |
...
| P0 | Vitrine | Re-record the promo video — not signed in as admin, clearer bounty preview | Current promo runs from the admin account, which betrays an internal-tester perspective. The bounty preview in the "working on this" segment doesn't read clearly. Pipeline: [`video/`](../video/) (`make promo`). Land before re-pinning the tweet on [@vidithq](https://x.com/vidithq) with the repo link. |
| P0 | UX | Highlight the Bounties sidebar nav on `/bounties/[id]` and `/bounties/new` | [`Sidebar.tsx:114`](../frontend/src/components/Sidebar.tsx:114) — the Bounties entry has no `activeFor`, so it falls through to `pathname === item.href` (lines 119–121) and never matches a bounty sub-route. Map nav demonstrates the pattern at lines 98–111. Fix: `activeFor: (p) => p.startsWith("/bounties")` — Submit owns `/geolocations/new`, not `/bounties/new`, so no exclusion needed. |
| P0 | Vitrine | "Before public launch" copy on `/about` reads stale on an already-public page | [`about/page.tsx:238`](../frontend/src/app/about/page.tsx:238) — "A full legal terms-of-service and privacy policy will land before public launch" sits on a page reachable without an account. The intended gate is M2 (open registration), not "public visibility." Reword to "before open registration" or "before Phase 3" so cold visitors don't read it as a pre-launch placeholder left in. |
| P1 | Vitrine | Replace the "Have an invite code?"-only CTA with a "Request access" affordance | [`HeroCtas.tsx:37`](../frontend/src/components/landing/HeroCtas.tsx:37) — the sole unsigned-out CTA presupposes the visitor already has an invite. After the pinned-tweet relaunch, cold traffic that wants in hits a dead-end. Add a "Request access" path: waitlist form, DM @vidithq, or "ask in Discord." |
| P1 | Vitrine | Fix Phase 3 stage conflation in landing copy | [`page.tsx:93`](../frontend/src/app/page.tsx:93) — landing treats "open beta" + "open registration" as one event; [`roadmap.md`](roadmap.md) carries them as two stages (3a open beta → 3b public v1). |
...
| P1 | Legal | DCO sign-off on inbound contributions | Legal-hygiene only — **not** a dual-licensing setup. The committed business model is hosted-service monetization on `vidit.app` (API rate limits + paid endpoints like alerting) and **no self-hosted licenses ever sold**. Customers hit the API as users, not operators, so AGPL §13 never binds them — this is the PostgreSQL / Mastodon shape (inbound = outbound = AGPL-3.0), not the MongoDB shape (which has a CLA only because it sells closed-source Enterprise binaries). What DCO buys: contributors certify they have the right to submit the code (didn't paste from a proprietary codebase, have employer permission, etc.), taking the maintainer out of the line of fire if someone uploads code they shouldn't have. Implementation: short "Contributor sign-off" section in [`CONTRIBUTING.md`](../CONTRIBUTING.md) + a `.github/workflows/dco.yml` (or similar) enforcing `Signed-off-by:` on each commit. **No CLA, no relicensing clause** — none of the friction that scares off privacy-conscious contributors (OSINT is privacy-conscious). Trade-off declined: forgoes acquisition optionality for a hypothetical buyer who'd pivot the company toward commercial-license monetization; consistent with the SAS à Mission framing. |
| P0 | Repo | README cold-read polish | A cold visitor on `github.com/vidithq/vidit` has to do the same first-impression job the landing does on `vidit.app`. Add: AGPL-3.0 license badge at the top; link to the live demo (`https://vidit.app`); link to the promo video; lift `make init / make seed / make dev` from prose into a fenced block right under *Getting started*; add a short "Why open source / why AGPL" blurb mirroring [`roadmap.md`](roadmap.md) → *Openness & transparency*; credit @geo27752 for demo-content consent. |
| P0 | Repo | Cross-link landing + about with the GitHub repo (and vice versa) | [`page.tsx:86`](../frontend/src/app/page.tsx:86) promises "the analyst platform becomes open source" but never links the repo. After the flip, every cold visitor reading the pitch must be one click from the code. Add an explicit "View on GitHub" link to the landing, the about page, and the sidebar footer; the repo's README equally needs to link the live app. |
| P0 | Repo | Open Graph + Twitter card metadata on `/` and `/about` | [`page.tsx`](../frontend/src/app/page.tsx) + [`about/page.tsx`](../frontend/src/app/about/page.tsx) — current `metadata` exports carry only `title` + `description`. The planned pinned tweet on [@vidithq](https://x.com/vidithq) will render a bland text card without `openGraph` + `twitter` blocks and an `og:image` (promo-video poster frame or map screenshot). About page is also `"use client"` with no `Metadata` export — wrap in a `layout.tsx` or split the client part off. |
| P0 | Repo | Set the GitHub repo description + topics before the public flip | First impression on the public OSS-listing page. Description: from [`vision.md`](vision.md) one-liner. Topics: `osint`, `geoint`, `geolocation`, `armed-conflict`, `nextjs`, `fastapi`, `postgis`, `agpl-3-0`, `open-source`. `gh repo edit vidithq/vidit --description "…"` + `gh api -X PUT repos/vidithq/vidit/topics -f names[]=osint …`. |
| P0 | Repo | Consolidate `docs/` into fewer, denser files | Per maintainer pref to reduce the set a newcomer has to wade through. Merges: `vision.md` + `roadmap.md` + `next.md` → **`docs/strategy.md`** (carries the elevator pitch, the 4 phases, milestone work tracker, "Explicitly out of scope" table, openness commitment — clearly delimited sections so cold readers can read the philosophy and skip the work tracker); `stack.md` + `architecture.md` → **`docs/engineering.md`** (tech choices + rationale + repo layout + deployment + particularities). Keep `design.md`, `data-model.md`, `api.md`, `backups.md` as-is (reference / ops domain). **Slim `api.md`** in the same pass: summary endpoint table at the top (method + path + auth + one-line purpose), fold rarely-touched admin endpoints into a collapsible section. **Touch points:** `.github/workflows/doc-sync.yml` pairings (`architecture.md` → `engineering.md`); *Doc sync rule* table in [`CLAUDE.md`](../CLAUDE.md); every internal cross-link across the repo + frontend (grep for `docs/vision.md`, `docs/roadmap.md`, `docs/next.md`, `docs/architecture.md`, `docs/stack.md`). **Caveat:** highest-effort row in M1 — budget a half-day. The wrong merge loses load-bearing context: the *Explicitly out of scope* table from `roadmap.md`, the *Refactors* and *Unscheduled candidates* sections from `next.md`, the *Particularities* block from `architecture.md` all need explicit homes in the merged files, not silent absorption. |
| P0 | Repo | Move root MDs (except README, CLAUDE, LICENSE) into `docs/` | GitHub's Community Standards detector reads `CONTRIBUTING.md`, `CODE_OF_CONDUCT.md`, `SECURITY.md` from root, `docs/`, OR `.github/` — moving to `docs/` keeps detection. Moves: `CHANGELOG.md` → `docs/CHANGELOG.md`; `CONTRIBUTING.md` → `docs/CONTRIBUTING.md`; `CODE_OF_CONDUCT.md` → `docs/CODE_OF_CONDUCT.md`; `SECURITY.md` → `docs/SECURITY.md`. `LICENSE` must stay at root (license auto-detection is root-only). Root tree shrinks to: `README.md`, `CLAUDE.md`, `LICENSE`, `Makefile`, `docker-compose.yml`, `.gitignore`, source dirs. **Touch points:** `.github/workflows/doc-sync.yml` (CHANGELOG path); every internal link to the moved files (CLAUDE.md, README, `CONTRIBUTING.md`'s own links, etc.); the per-mailbox security contact in `SECURITY.md` stays as-is. **Verify** post-push: GitHub repo Insights → Community Standards page shows all four still detected; "missing" reads = path or filename wrong. |
| P0 | Repo | Single source of truth via `/content/` for landing, about, and strategy copy | Today the "what is Vidit / why it exists / how it's open" paragraphs live in three places that drift: [`page.tsx`](../frontend/src/app/page.tsx) + [`about/page.tsx`](../frontend/src/app/about/page.tsx) (website) and [`vision.md`](vision.md) (post-merge: `strategy.md`). A strategy update needs three edits; drift compounds. **Implementation:** create `/content/` at repo root with discrete markdown chunks — `content/pitch.md` (one-paragraph elevator pitch), `content/mission.md` (producer / extractor framing), `content/openness.md` (AGPL + radical-transparency commitment), `content/about-team.md` (team + funding framing for `/about`). Render them in the website via a small `<MarkdownContent name="…" />` component (a thin `react-markdown` wrapper — no MDX setup needed; markdown files imported as raw strings via Next.js's `import` of `.md`). `docs/strategy.md` consumes the same `/content/*.md` files: either via a build-time stitcher script that inlines them into the rendered file, or via clearly delimited "`<!-- include: content/pitch.md -->`" markers enforced by a doc-sync CI rule. Pick the mechanism at implementation time. **New doc-sync CI rule:** changes to `/content/**` must touch the website page (or pass a rebuild check); changes to website strategy-copy paragraphs that don't touch `/content/` fail. **Trade-off:** ~3–4 hours of one-time engineering for the extraction + component + CI rule; kills the drift problem permanently and removes the "match the docs by hand" failure mode from every future strategy edit. |
| P0 | Repo | Verbosity + repetition pass on website + docs | Tighten prose across both surfaces. Three failure modes to hunt: (a) **long phrases** saying in 20 words what 8 would do (the existing copy carries audit-shaped sentences with multiple subclauses where a single declarative would land harder); (b) **cross-doc repetition** — the "100% open source / no proprietary tier / AGPL keeps hosted forks open" framing today appears with subtle variation across [`roadmap.md`](roadmap.md) (*Openness & transparency*), [`CONTRIBUTING.md`](../CONTRIBUTING.md) (*Open posture*), the landing, and `/about`; pick one canonical phrasing in `content/openness.md` and reference everywhere else; (c) **intra-doc repetition** where a concept gets re-explained at each callsite instead of cross-linked once. Apply to `/content/*.md` (highest leverage — these paragraphs surface on the website where verbosity costs reader trust), `docs/strategy.md` (the merge will expose duplicates that the split files were hiding), `docs/api.md` (prune endpoint prose once the summary table lands), [`README.md`](../README.md). Heuristic: cut ~25% on first pass, reread cold, restore anywhere meaning suffered. **Sequence after Rows 1 and 3** — don't tighten text that's about to be moved or merged. |
| P1 | Vitrine | Founder-identity pass on `/about` | Two cold-impression issues. (1) [`about/page.tsx:183`](../frontend/src/app/about/page.tsx:183) leads with "hobby project, costs out of pocket" — the openness is good, but leading with fragility undermines trust for first-time visitors. Reorder so the mission + AGPL commitment lead, the funding-honesty paragraph trails as transparency. (2) [`about/page.tsx:249`](../frontend/src/app/about/page.tsx:249) "a solo European developer" reads cagey to OSINT analysts who themselves trade on public identity ([`roadmap.md`](roadmap.md) even cites "established X handle" as a trust signal). Decision call: name + public X handle, or stay anonymous and own it explicitly — letting the placeholder ride splits the difference badly. |
| P2 | Repo | `.github/FUNDING.yml` decision | Optional. Wire to GitHub Sponsors / Open Collective / Liberapay if you want a "Sponsor" button on the repo page. Skip cleanly if monetization stays purely hosted-tier API throughput — the `FUNDING.yml` convention assumes donations, which isn't the model. |
...
## M3 — Public v1 (full release)

Stage 3b of [Phase 3](roadmap.md). The open beta proven out and finished into v1: real catalog density, search and social depth, cost tuning, and the closed-beta framing removed. Open source (M1) has already landed by here. Priorities are relative within the milestone — P1 before P2; none of it blocks the open-beta gate.
