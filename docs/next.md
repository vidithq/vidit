...
Forward-looking work list, ordered by milestone. Items get deleted when they ship — the [CHANGELOG](CHANGELOG.md) records what landed.

The path from closed beta to public v1 is three milestones. The throughline is **progressive openness**: open the source + widen the closed-beta analyst pool in a single coordinated launch, open the doors (M2), call it done (M3). Reach is the current bottleneck and the community is skeptical of closed / unknown tools.
...
The single coordinated launch that retires the "closed-source / unknown / vibe-coded" objection and widens the closed-beta analyst pool beyond the early testers. The landing page already ships (the public vitrine + the demo video — see [CHANGELOG](CHANGELOG.md) under *Unreleased* and *v0.2.0*); what's left is the open-source flip + the pinned X tweet on [`@vidithq`](https://x.com/vidithq) + cold-reach DMs to new analysts, all firing in the same window. **100% open source under [AGPL-3.0](../LICENSE)** — no proprietary tier; the monetization layer on `vidit.app` is API rate limits + a small set of paid-only endpoints (see [`roadmap.md`](roadmap.md) → *Openness & transparency*). Independent of the M2 safety stack.
...
| P0 | Repo | README cold-read polish | Add: AGPL-3.0 license badge at the top; link to the live demo (`https://vidit.app`); link to the promo video; short "Why open source / why AGPL" blurb mirroring [`roadmap.md`](roadmap.md) → *Openness & transparency*; credit @geo27752 for demo-content consent. |
| P0 | Repo | Cross-link landing + about with the GitHub repo (and vice versa) | [`page.tsx:86`](../frontend/src/app/page.tsx:86) promises "the analyst platform becomes open source" but never links the repo. Add an explicit "View on GitHub" link to the landing, the about page, and the sidebar footer; the repo's README equally needs to link the live app. |
...
| P0 | Repo | Set the GitHub repo description + topics before the public flip | Description: from [`roadmap.md`](roadmap.md) one-liner. Topics: `osint`, `geoint`, `geolocation`, `armed-conflict`, `nextjs`, `fastapi`, `postgis`, `agpl-3-0`, `open-source`. `gh repo edit vidithq/vidit --description "…"` + `gh api -X PUT repos/vidithq/vidit/topics -f names[]=osint …`. |
| P1 | Vitrine | Founder-identity pass on `/about` | (1) [`about/page.tsx:183`](../frontend/src/app/about/page.tsx:183) leads with "hobby project, costs out of pocket" — leading with fragility undermines trust. Reorder so the mission + AGPL commitment lead, the funding-honesty paragraph trails as transparency. (2) [`about/page.tsx:249`](../frontend/src/app/about/page.tsx:249) "a solo European developer" reads cagey to OSINT analysts who themselves trade on public identity ([`roadmap.md`](roadmap.md) even cites "established X handle" as a trust signal). Decision call: name + public X handle, or stay anonymous and own it explicitly. |
| P2 | Repo | `.github/FUNDING.yml` decision | Optional. Wire to GitHub Sponsors / Open Collective / Liberapay if you want a "Sponsor" button on the repo page. Skip if monetization stays hosted-tier API throughput — `FUNDING.yml` assumes donations. |
...
Stage 3b of [Phase 3](roadmap.md). Open beta hardened into v1: catalog density, search and social depth, cost tuning, closed-beta framing removed. Priorities are relative within the milestone — P1 before P2; none of it blocks the open-beta gate.
...
The platform's quality model rests on two things: the admin-curated `trusted_contributor` flag (with a reader-facing filter), and per-action moderation. There is no in-platform reputation math — explicitly out of scope per [`roadmap.md`](roadmap.md).

Lightweight gamification could reinforce engagement without leaking into the trust signal. Bounties (scheduled above) are the natural place to start.
...
**Rule for if any of this ships:** gamification primitives stay **separate** from the trust signal. Scores, badges, and streaks should never influence whether someone gets the `trusted_contributor` checkmark, and they should never appear in the same UI surface as the trust filter.
...
Pick up reactively. The quarterly drill stays the canonical "is the backup real" check.
...
Three reaper jobs ship as on-demand admin-panel buttons today: `reap_auth_tokens` (expired sessions / refresh tokens), `reap_proof_image_orphans` (Tiptap inline images not referenced by any geolocation proof), and `reap_pending_registrations` (abandoned signups past the 24h TTL). Clicking the button when checking the dashboard is fine for now. Schedule them via a Railway cron service (parallel to the backup cron). Single-file change once it matters — the reaper functions already exist server-side; only the Railway cron schedule + Dockerfile shim is missing.
...
- DNS: add a record for `status.vidit.app` and flip it to **Proxied** (orange cloud). Safe to proxy this one because the subdomain hosts no service of its own and doesn't need Let's Encrypt — unlike the apex and `api.` where proxy mode breaks cert provisioning (see [`engineering.md`](engineering.md) → *Particularities*).
...
Worth wiring once the status page becomes something analysts are asked to check (linked from `/about`, pinned in Discord); not urgent until then.

### Mobile responsiveness audit

A mobile pass across the golden paths (map → detail → submit → profile → admin under 375px) was scoped once, then de-prioritized: desktop is the only target surface for the closed beta. The audit can come back if and when public registration is on the table (Phase 3), where casual visitors on phones become a real population.
