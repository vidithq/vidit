...
The path from closed beta to public v1 is three milestones. The throughline is **progressive openness**: open the source + restart cold reach in a single coordinated launch (M1 — the landing already ships, so the GitHub flip + pinned X tweet + first analyst DMs all fire in the same window), open the doors (M2), call it done (M3). Reach is the current bottleneck and the community is skeptical of closed / unknown tools — M1 is the load-bearing trust signal.

| Milestone | What it delivers | What it unlocks |
|---|---|---|
| **M1 — Open source launch & first analysts** *(now)* | Public vitrine at `vidit.app` already ships (landing + demo video); the open-source flip under AGPL-3.0, the pinned X tweet on `@vidithq`, and the first cold-reach DMs to analysts fire as one coordinated launch | A skeptic clicking the tweet sees a working product and the code in one go; the "closed-source / vibe-coded" objection retires; first analyst invites land in the same window |
...
## M1 — Open source launch & first analysts *(now)*

The single coordinated launch that retires the "closed-source / unknown / vibe-coded" objection and restarts cold reach to analysts. The landing page already ships (the public vitrine + the demo video — see [CHANGELOG](../CHANGELOG.md) under *Unreleased* and *v0.2.0*); what's left is the open-source flip + the pinned X tweet on [`@vidithq`](https://x.com/vidithq) + the first analyst DMs, all firing in the same window because the trust signal compounds when substance + code + presence arrive together. **100% open source under [AGPL-3.0](../LICENSE)** — no proprietary tier; the monetization layer on `vidit.app` is API rate limits + a small set of paid-only endpoints (see [`roadmap.md`](roadmap.md) → *Openness & transparency*). Orthogonal to the M2 (open beta) safety stack — this can land before that work begins.
