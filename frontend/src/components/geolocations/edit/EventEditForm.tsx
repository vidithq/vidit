 ...
 *
 * Built like the create form throughout: the shared `EventFormFields` block and
 * the `useEventForm` state behind it, `MediaManager` staging included. State is
 * seeded from props (the form mounts only after the row loaded), so the Tiptap
 * editor gets its `initialContent` on first paint.
 *
...
export function EventEditForm({
  ...
  // Every field the two writes share, seeded from the row: the form mounts only
  // after it loaded, so the Tiptap editor gets its `initialContent` on first
  // paint.
