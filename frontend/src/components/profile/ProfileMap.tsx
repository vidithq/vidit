...
import { WORLD_BBOX, pointsBounds } from "@/components/map/bounds";
...
export function ProfileMap({ username }: { username: string }) {
  ...
  // an ineligible one 422s server-side.
  const path = AUTHOR_FILTER_RE.test(username)
    ? `/events/points?author=${encodeURIComponent(username)}&bbox=${WORLD_BBOX}`
    : null;
  ...
  return (
    <Card as="section">
      <div className="flex items-center justify-between gap-3">
        <SectionEyebrow title="Coverage" as="h3" margin="none" />
