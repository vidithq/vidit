...
|---|---|---|---|
| P0 | Ingestion | A — On-demand X API pull per consented handle | Discovery step, **app-only bearer** — reads public tweets, asks nothing of the analyst. Paid but bounded: one handle at a time, on demand (~$1 per ~200 tweets at 2026 X API Basic per-result read pricing). Only consented handles are ever pulled. |
| P0 | Ingestion | A — "Is this a geolocation tweet?" classifier | Binary filter over the pulled timeline. LLM-based; narrows a handle's tweets down to the geolocation ones. **Spike, sequenced:** labelled eval set first — it gates both the approach choice and the precision/recall bar. Baseline to beat: the deterministic coordinate extraction already in [`tweet_parsing.py`](../backend/app/services/tweet_parsing.py) (a parseable coord ⇒ almost certainly a geo tweet) — measure LLM lift over that, don't assume a from-scratch classifier. Still open: the geolocation-tweet definition, the P/R bar, the output contract. First LLM integration in the backend. |
...
| P0 | Data model | A — Split `Author` from `User` | Today fused: `geolocations.author_id` is a NOT NULL FK to `users.id`, so no profile exists without an auth account. Decouple so an assembled profile exists before its owner logs in. **Decided:** claim-flag on `users`, not a new `authors` table — the assembled profile IS a credential-less `users` row (`email` / `password_hash` now nullable), anchored by a unique `x_handle`, with `claimed_at` NULL until an owner takes it. `author_id` keeps pointing at `users.id`: no FK re-point, no author-query rewrite, and existing hand-made profiles (incl. demo authors) are already `users` rows so nothing migrates. Foundational columns + backfill in migration `u7p9r1t3v5x7`; the consent gate, assembly pipeline, and claim flow (rows below) build on it. |
...
| P0 | Onboarding | B — Pipeline assembles the profile from the handle | On consent: handle → API pull → classifier → structured geolocations (syndication) → a profile + its geolocations, ready for the owner with zero data entry on their part. Normalize `x_handle` on write (lowercase, strip `@`) — the column's UNIQUE dedup silently mints duplicates otherwise. |
