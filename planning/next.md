...
| P0 | Repo | Embed the promo MP4 in the README via a GitHub user-attachment URL | The README's *Demo* section currently links to the live `.mp4` on `vidit.app`. After the next [`make promo`](../video/) re-render lands, drag the resulting `video/out/promo-final.mp4` into a draft GitHub issue or release, copy the generated `https://github.com/user-attachments/...` URL, and paste it on a line by itself in the *Demo* section of [`README.md`](../README.md) so GitHub renders the player in-page. Sequence: lands alongside the [`re-render the promo`](#m1--open-source-launch-now) row above. |
