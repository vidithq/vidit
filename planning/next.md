...
| P1 | Code hygiene | Dead-code & misconception sweep — delete config/code that no longer does anything; fix comments/docs that describe behaviour the code lacks | The [#55](https://github.com/vidithq/vidit/pull/55) rate-limiting pass surfaced a class worth hunting before the flip: a `default_limits` global that enforced nothing (no middleware behind it), a `rate_limit_enabled` toggle most routers silently ignored, and comments in three files reasoning from a 60/min floor that never existed. Sweep for the same shape — settings read by nothing or only by dead branches, exception handlers for errors that can't fire, helpers/branches no `git grep` reaches, and prose (comments, docstrings, `docs/*.md`) asserting behaviour the code doesn't implement. Delete the dead, correct the false; where a no-op is load-bearing by accident (something leans on the phantom), make it real or drop the dependency. `vulture` + `ruff --select F401,F841` + `ts-prune` seed the mechanical half; the misconception half is a read-and-verify pass. Visible to every public reader who audits the source; do it before the flip. |
