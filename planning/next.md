...
**The Phase A detection spine has landed** ([CHANGELOG](../CHANGELOG.md) → *Unreleased*): the state machine, the acquire / stitch / detect bricks, `assemble`, the archive backfill (dev-fed by a committed *synthetic* archive via `make seed-detections`), `detected` rendered marked on every surface, and the no-persist preview. The rows below are what's left for v0.4 — the **analyst on-ramp** (OAuth handle-ownership, the archive upload UI + tweets-only guard, the owner review queue, the bot), the **typology / recall QA harness**, and the **legal + release** gates. The synthetic archive is the dev-only feeder; the real analyst upload is the deferred on-ramp.
...
| P0 | Review | B — Owner review queue UI | The owner's queue of their `detected` geolocations — edit / complete / validate / reject inline, over the shipped state-gated endpoints (`PATCH /geolocations/{id}`, `POST .../validate`, `POST .../reject`). The on-ramp lands here after an import; it's where `detected` becomes `validated` over time. |
