...
| P1 | Frontend | Vitest + Testing Library harness before the page splits below | The splits below refactor 1,000+-line components with zero frontend tests in the repo — no runner, no `test` script; CI is lint + `tsc` + build. Land the harness first, seeded with the highest-value pure units: the `safeNext` open-redirect guard (`frontend/src/app/(auth)/login/page.tsx:42`), `splitHighlights` ([`lib/search.ts:45`](../frontend/src/lib/search.ts)), [`lib/mediaUrls.ts`](../frontend/src/lib/mediaUrls.ts), the proof renderer ([`lib/proof.tsx`](../frontend/src/lib/proof.tsx)) — security-relevant logic currently protected only by the type-checker — and [`hooks/useApiResource.ts`](../frontend/src/hooks/useApiResource.ts) (path-change abort, null-path skip, refetch-after-error vs refetch-after-success), which every page's data loading now routes through. The splits then refactor under a net. |
