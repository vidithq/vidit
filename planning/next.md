...
| P0 | Vitrine | Re-render the promo via `make promo` and upload the new MP4 to CloudFront | The recording pipeline now logs in as `analyst@vidit.app` instead of admin (see [`video/record-submit.js`](../video/record-submit.js), [`backend/scripts/mock_demo_user.py`](../backend/scripts/mock_demo_user.py)). Outstanding: run `make promo` locally, verify `video/out/promo-final.mp4`, and `aws s3 cp` to the CloudFront-fronted bucket so `NEXT_PUBLIC_DEMO_VIDEO_URL` resolves to the new cut. Caption sharpening on the "I'm working on this" beat ([`video/src/Demo.tsx`](../video/src/Demo.tsx) `CAPTIONS`) is a likely follow-up after eyeballing the new recording. |
...
| P0 | Vitrine | "Before public launch" copy on `/about` reads stale on an already-public page | Two occurrences, both on a page reachable without an account ([`middleware.ts:32`](../frontend/src/middleware.ts:32) allowlist): [`about/page.tsx:238`](../frontend/src/app/about/page.tsx:238) "A full legal terms-of-service and privacy policy will land before public launch" and [`about/page.tsx:25`](../frontend/src/app/about/page.tsx:25) "We'll formalise the licence terms before public launch…". The intended gate is M2 (open registration), not "public visibility." Reword both to "before open registration". |
...
| P0 | Repo | Cross-link landing + about with the GitHub repo (and vice versa) | [`page.tsx:80`](../frontend/src/app/page.tsx:80) promises "Vidit becomes open source under AGPL-3.0" but never links the repo. Add an explicit "View on GitHub" link in three places: (1) landing, near the "Open source launch · Now" roadmap card; (2) about page, in the AGPL / transparency paragraph; (3) sidebar **header**, as a 3rd `<a>` next to the existing X + Discord shortcuts at [`Sidebar.tsx:229-248`](../frontend/src/components/Sidebar.tsx:229) (define `GITHUB_URL = "https://github.com/vidithq/vidit"` alongside the `X_URL` / `DISCORD_URL` constants, add a `GitHubGlyph` component, reuse the existing icon-link styling). The reverse link (README → live app) is covered by the README polish row above. |
...
| P1 | Vitrine | Founder-identity pass on `/about` | (1) [`about/page.tsx:183`](../frontend/src/app/about/page.tsx:183) — within the Funding section (mission + AGPL commitment already lead the page via Commitments at L131), the opening paragraph "Today, Vidit is a hobby project. All running costs come out of one person's pocket" leads with fragility before stating the longer-term ambition. Reorder the three paragraphs intra-section: long-term independence + revenue-back-to-analysts ambition first, the "hobby project / out of pocket" honesty paragraph last as transparency. (2) [`about/page.tsx:249`](../frontend/src/app/about/page.tsx:249) "a solo European developer" reads cagey to OSINT analysts who themselves trade on public identity. **Decision: anonymous + explicitly owned** — rewrite L249 along the lines of "maintained under operational security similar to the analysts the platform serves; trust rests on the AGPL guarantee + auditable codebase, not on the maintainer's identity." |
