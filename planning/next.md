...
| P1 | Frontend | Split [`app/profile/[username]/page.tsx`](../frontend/src/app/profile/[username]/page.tsx) (573 lines) | Single component mixes profile display + inline edit mode + recent submissions list + follow button + external-links editor. Extract `<ProfileHeader>`, `<ProfileEditForm>`, `<RecentSubmissions>` under `components/profile/`. Trips the same `useApiResource` quartet as the worst sites above ([`profile/[username]/page.tsx:73-75`](../frontend/src/app/profile/[username]/page.tsx:73)) — once that hook lands, consume it from the extracted children. |
| P1 | Backend | S3 orphan-sweep helper | `try: storage.delete_many(keys) except StorageDeleteError: logger.exception(...)` recurs 5 times across [`routers/bounties.py`](../backend/app/routers/bounties.py) + [`routers/geolocations.py`](../backend/app/routers/geolocations.py). Two of these also do the same `uploaded_keys += derivative_keys` collection loop upstream. Add a `sweep_keys(keys, context=...)` helper in [`services/storage.py`](../backend/app/services/storage.py) that wraps the try/except/log. |
| P1 | Backend | Extract [`routers/geolocations.py::create_geolocation`](../backend/app/routers/geolocations.py:949) (~340 lines, runs 949 → 1289) into a service | Single endpoint with business logic in the router: upload orchestration, error sweep, image derivative generation, proof image moves. Violates the [AGENTS.md](../AGENTS.md) "routers → services → models, no business logic in routers" convention. Move the body into a `create_with_evidence(...)` function in a new `services/geolocations.py`. Lands cleanly after the S3 sweep helper above since the new service consumes `sweep_keys(...)` directly. |
| P1 | Frontend | Split map [`app/map/page.tsx`](../frontend/src/app/map/page.tsx) (547 lines) + shared `<GeolocationDetailBody>` | Interleaves map render + filter panel (~140 lines) + detail side-panel (~200 lines) + data loading. Extract `<FilterPanel>` and `<DetailSidePanel>` into `components/map/`. The side-panel currently re-renders the geolocation markup that [`geolocations/[id]/page.tsx`](../frontend/src/app/geolocations/[id]/page.tsx) also renders — extract a shared `<GeolocationDetailBody>` in the same PR so the split is paired with the real de-duplication win. |
